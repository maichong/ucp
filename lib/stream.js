...
class Stream extends EventEmitter {
  ...
  _write() {
    ...
    this.sentLength++;
    let now = Date.now();
    if (!this._lastLog || now - this._lastLog > 600) {
      console.log(this._writeQueue.length, this.sentLength);
      this._lastLog = now;
    }
    if (this.sentLength < 200) {
      this._write();
    } else {
      this.writeTimer = setTimeout(() => this._write(), 3);
    }
  }
