...
class Stream extends EventEmitter {
  ...
  constructor(id, connection) {
    ...
    this._aliveTimer = setInterval(() => {
      if (Date.now() - this.lastAlive > 200000) {
        this.destroy();
      }
    }, 5000);

    //确认Timer
    this._confirmTimer = setInterval(() => this._confirm(), 10);
  ...
  _write() {
    ...
    this.sentLength++;
    if (this.sentLength < rto * 3) {
      this._write();
    } else if (this.sentLength < rto * 6) {
      this._writeTimer = setTimeout(() => this._write(), parseInt(rto / 100));
    } else {
      this._writeTimer = setTimeout(() => this._write(), 5);
    }
  }
