...
const UCP_BUS_DELAY = config('UCP_BUS_DELAY', 6, 'int');
const UCP_BUS_MAX_LENGTH = config('UCP_BUS_MAX_LENGTH', 1450, 'int');

class Bus {
  constructor(packet) {
    this.departure = Date.now() + UCP_BUS_DELAY;
    this.packets = [];
    this.length = 0;
    if (packet) {
  ...
  ride(packet) {
    if (packet.buffer.length + this.length >= UCP_BUS_MAX_LENGTH) {
      //如果过长,不允许搭车
      return false;
    }
